...
Если хотя бы одна из сумм: $(k + k')$, $(m + m')$, $(n + n')$ --- нечетна, то весь интеграл равен нулю. Иначе:
$$ I_{ij}(k,k') = \frac{(k+k'-1)!!\sqrt{\pi}}{2^{(k+k')/2}(\alpha_i+\alpha_j)^{(k+k'+1)/2}}$$
$$ I_{ij}(k,m,n,k',m',n') = \frac{(k+k'-1)!!(m+m'-1)!!(n+n'-1)!!\pi^{3/2}}{2^{(L+L')/2}(\alpha_i+\alpha_j)^{(L+L'+3)/2}}$$
где $L = k+m+n,\ L' = k' + m' + n'$
